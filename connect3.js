...
async function query() {
  ...
  let tal2Schema = mongoose.Schema({
    身份证号: String,
    性别: String,
    户籍地代码: String,
    录取院校代码: String,
  ...
  let tal2NewSchema = new mongoose.Schema({
    身份证号: String,
    性别: String,
    户籍地代码: String,
    录取院校代码: String,
  ...
  let tal2NewForm = mongoose.model(
    '高考库关联社保个人库单位库-省外生源',
    tal2NewSchema,
    '高考库关联社保个人库单位库-省外生源'
  );
  ...
  let count = 1;
  const cursor = tal2Form.find({}).cursor();
  let doc;
  while ((doc = await cursor.next())) {
    ...
    const psn = new tal2NewForm(person);
    await psn.save();
    console.log(`${count++}`);
  }
